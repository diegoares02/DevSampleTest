...
namespace DevSample
{
    class Program
    {
        ...
        static readonly string folderPath = @"C:\Temp\";
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < _cyclesToRun; i++)
            {
                try
                {
                    ...
                    string valueSum = sampleGenerator.Samples.Sum(s => (decimal)s.Value).ToString("N");

                    //TODO: why do we only seem to get 7 digits of precision? The CEO wants to see at least 20!
                    LogMessage($"Cycle {i} Sum of All Samples: {valueSum}.");
        ...
        static void LogToFile(string message)
        {
            ...
            using (StreamWriter stream = new StreamWriter(folderPath + fileName, true))
            {
